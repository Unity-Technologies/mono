...
should_skip_property_evaluation (MonoMethod *method, MonoClass *debugger_browsable_class)
{
	...
	/* Check if the property has [DebuggerBrowsable(Never)] */
	ERROR_DECL (error);
	MonoCustomAttrInfo *ainfo = mono_custom_attrs_from_property_checked (method->klass, prop, error);
	
	if (is_ok (error) && check_debugger_browsable_never (ainfo, debugger_browsable_class)) {
		PRINT_DEBUG_MSG (1, "[%p] Property '%s.%s' has [DebuggerBrowsable(Never)].\n",
			(gpointer) (gsize) mono_native_thread_id_get (),
